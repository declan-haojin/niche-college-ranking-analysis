...
Average SAT (`SAT_AVG`), median ACT (`ACTCMMID`), graduation rate (`C150_4`), average faculty salary (`AVGFACSAL`), and admission rate (`ADM_RATE`), have the strongest relationship with rank because their R-squared values are above 0.4. Additionally, the largest sequential gap between R-squared values occurs between admission rate and the following variable, percentage of students receiving Pell Grants (`PCTPELL`).
