...
The second dataset comes from the US Department of Education's College Scorecard from the 2020-2021 school year. There were `r ncol(us_dep_of_ed)` variables in the original data set, but we narrowed to 30 variables we thought could have an influence on rank. We left variables out to avoid redundancy, such as test score breakdowns. There are `r nrow(us_dep_of_ed)` observations in the data set, representing all of the colleges and universities in the US.
...
2.  `University of South Florida-Sarasota-Manatee` and `University of South Florida-St. Petersburg` were removed because they did not exist in both datasets due to naming discrepancies.
...
3.  All of the numerical variables are on different scales, so we created scaled version of the numerical explanatory variables, with mean 0 and standard deviation 1.
...
The plot above visually displays the decreasing AIC value as variables get taken out added back. Each point represents a new iteration of the model after a variable is taken out (-) or added (+). The final iteration is the best model because its AIC value is the lowest.
