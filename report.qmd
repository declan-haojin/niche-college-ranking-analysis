...
3.  We joined the US Department of Education dataset to the Niche dataset by the college names. The first 5 rows are displayed below.
...
`COSTT4_P` Average cost of attendance for program-year institutions) has an R-squared value of 1 because there are only two observations with entries for it. Therefore, we will not be using it in the stepwise regression model. Based on the r-squared values of the remaining variables, we expect SAT_AVG (average SAT score), ACTCMMID (median ACT score), and C150_4 (rate of completion) to appear in the final model because they all have an R-squared value above 0.5, meaning that they explain more than 50% of the variation seen in rank.
...
#| label: admission_slope_r2

#START

#ADM_RATE
s_adm_rate_rank_reg <- linear_reg() |>
  fit(rank ~ ADM_RATE, data = scaled_data)
s_adm_rate_rank_reg

adm_rate_rank_reg <- linear_reg() |>
  fit(rank ~ ADM_RATE, data = colleges)
adm_rate_rank_reg

glance(adm_rate_rank_reg) |>
  select(r.squared)

#SAT_AVG
s_SAT_AVG_rank_reg <- linear_reg() |>
  fit(rank ~ SAT_AVG, data = scaled_data)
s_SAT_AVG_rank_reg

SAT_AVG_rank_reg <- linear_reg() |>
  fit(rank ~ SAT_AVG, data = colleges)
SAT_AVG_rank_reg

glance(SAT_AVG_rank_reg) |>
  select(r.squared)

#ACTCMMID
s_ACTCMMID_rank_reg <- linear_reg() |>
  fit(rank ~ ACTCMMID, data = scaled_data)
s_ACTCMMID_rank_reg


ACTCMMID_rank_reg <- linear_reg() |>
  fit(rank ~ ACTCMMID, data = colleges)
ACTCMMID_rank_reg

glance(ACTCMMID_rank_reg) |>
  select(r.squared)

#create tibble
admission_slope_r2 <- tibble(
  "Explanatory Variable" = c(
    "Admission Rate",
    "SAT Average",
    "ACT Median"
  ),
  "Standarized Slope" = c("91.74", "-115.0", "-112.9"),
  "Slope" = c("387.02", "-0.9277", "-30.04"),
  "R-Squared" = c("0.405", "0.629", "0.606")
)

admission_slope_r2 |> knitr::kable() 
```

```{r}

#| label: create_tibble_admissions
...
#| label: demographics_graphs

race_scaled_colleges <- colleges |> 
  mutate(
    scaled_UGDS_WHITE = scaled_data$UGDS_WHITE,
    scaled_UGDS_BLACK = scaled_data$UGDS_BLACK,
    scaled_UGDS_HISP = scaled_data$UGDS_HISP,
    scaled_UGDS_ASIAN = scaled_data$UGDS_ASIAN,
    scaled_UGDS_NRA = scaled_data$UGDS_NRA
  )

b1 <- race_scaled_colleges |>
  filter(scaled_UGDS_WHITE >= -3 & scaled_UGDS_WHITE <= 3) |> 
  ggplot(aes(x = UGDS_WHITE, y = log(rank))) +
...
b2 <- race_scaled_colleges  |>
  filter(scaled_UGDS_BLACK >= -3 & scaled_UGDS_BLACK <= 3) |> 
...
b3 <- race_scaled_colleges |>
  filter(scaled_UGDS_HISP >= -3 & scaled_UGDS_HISP <= 3) |> 
  ggplot(aes(x = UGDS_HISP, y = log(rank))) +
  geom_smooth(method = "lm", se = F) +
  geom_point(alpha = 0.25) +
  labs(
    x = "% Hispanic",
...
b4 <- race_scaled_colleges |> 
  filter(scaled_UGDS_ASIAN >= -3 & scaled_UGDS_ASIAN <= 3) |> 
...
b5 <- race_scaled_colleges |> 
  filter(scaled_UGDS_NRA >= -3 & scaled_UGDS_NRA <= 3) |> 
  ggplot(aes(x = UGDS_NRA, y = log(rank))) +
  geom_smooth(method = "lm", se = F) +
  geom_point(alpha = 0.25) +
  labs(
...
#| label: demographics_slope_r2

#START

#UGDS_WHITE
s_UGDS_WHITE_rank_reg <- linear_reg() |>
  fit(log(rank) ~ UGDS_WHITE, data = scaled_data)
s_UGDS_WHITE_rank_reg

UGDS_WHITE_rank_reg <- linear_reg() |>
  fit(log(rank) ~ UGDS_WHITE, data = colleges)
UGDS_WHITE_rank_reg

glance(UGDS_WHITE_rank_reg) |>
  select(r.squared)

#UGDS_BLACK
s_UGDS_BLACK_rank_reg <- linear_reg() |>
  fit(log(rank) ~ UGDS_BLACK, data = scaled_data)
s_UGDS_BLACK_rank_reg

UGDS_BLACK_rank_reg <- linear_reg() |>
  fit(log(rank) ~ UGDS_BLACK, data = colleges)
UGDS_BLACK_rank_reg

glance(UGDS_BLACK_rank_reg) |>
  select(r.squared)

#UGDS_HISP
s_UGDS_HISP_rank_reg <- linear_reg() |>
  fit(log(rank) ~ UGDS_HISP, data = scaled_data)
s_UGDS_HISP_rank_reg

UGDS_HISP_rank_reg <- linear_reg() |>
  fit(log(rank) ~ UGDS_HISP, data = colleges)
UGDS_HISP_rank_reg

glance(UGDS_HISP_rank_reg) |>
  select(r.squared)

#UGDS_ASIAN
s_UGDS_ASIAN_rank_reg <- linear_reg() |>
  fit(log(rank) ~ UGDS_ASIAN, data = scaled_data)
s_UGDS_ASIAN_rank_reg

UGDS_ASIAN_rank_reg <- linear_reg() |>
  fit(log(rank) ~ UGDS_ASIAN, data = colleges)
UGDS_ASIAN_rank_reg

glance(UGDS_ASIAN_rank_reg) |>
  select(r.squared)

#UGDS_NRA
s_UGDS_NRA_rank_reg <- linear_reg() |>
  fit(log(rank) ~ UGDS_NRA, data = scaled_data)
s_UGDS_NRA_rank_reg

UGDS_NRA_rank_reg <- linear_reg() |>
  fit(log(rank) ~ UGDS_NRA, data = colleges)
UGDS_NRA_rank_reg

glance(UGDS_NRA_rank_reg) |>
  select(r.squared)
```

```{r}

#| label: create_tibble_demographics
...
admission_slope_r2 <- tibble(
  ...
  "Standardized Slope" = c("0.264", "0.1511", "0.03403", "-0.521", "-0.4448"),
  "Slope" = c("1.406 ", "1.906", "0.3083", "-6.569", "-8.517"),
  "R-Squared" = c("0.07280404", "0.02382938", "0.001209544", "0.2834427", "0.2066353")
)
```

```{r}

#| label: finanicals_graphs

finanicals_scaled_colleges <- colleges |> 
  mutate(
    scaled_NPT4_PUB = scaled_data$NPT4_PUB,
    scaled_NPT4_PRIV = scaled_data$NPT4_PRIV,
    scaled_COSTT4_A = scaled_data$COSTT4_A,
    scaled_AVGFACSAL = scaled_data$AVGFACSAL,
    scaled_ENDOWBEGIN = scaled_data$ENDOWBEGIN
  )
...
c2 <- colleges |> 
  ...
  geom_smooth(method = "lm", se = F) +
  labs(
    x = "Avg Net Price (private)",
    y = "Rank"
  )

c3 <- colleges |> 
  filter(CONTROL == 1) |> 
  ggplot(aes(x = COSTT4_A, y = rank)) +
  geom_point(alpha = 0.25, show.legend = F) +
  geom_smooth(method = "lm", se = F, show.legend = F) +
  labs(
    x = "Cost of Attendance (public)", 
    y = "Rank"
  ) 
...
c6 <- colleges |> 
  ggplot(aes(x = log(ENDOWBEGIN), y = rank)) +
  geom_point(alpha = 0.25) +
  geom_smooth(method = "lm", se = F) +
  labs(
    x = "Endowment Size", 
...
#| label: financials_slope_r2

#START

#NPT4_PUB
linear_reg() |>
  fit(rank ~ NPT4_PUB, data = scaled_data)

reg <- linear_reg() |>
  fit(rank ~ NPT4_PUB, data = colleges)
reg
...
#NPT4_PRIV
linear_reg() |>
  fit(rank ~ NPT4_PRIV, data = scaled_data)

reg <- linear_reg() |>
  fit(rank ~ NPT4_PRIV, data = colleges)
reg
...
#COSTT4_A-Pub
linear_reg() |>
  fit(rank ~ COSTT4_A, data = scaled_data |> filter(CONTROL == 1))

reg <- linear_reg() |>
  fit(rank ~ COSTT4_A, data = colleges |> filter(CONTROL == 1))
reg
...
#COSTT4_A-Priv
#linear_reg() |>
  #fit(rank ~ COSTT4_A, data = scaled_data |> filter(CONTROL == #2))

#reg <- linear_reg() |>
  #fit(rank ~ COSTT4_A, data = colleges |> filter(CONTROL == 2))
#reg
...
#AVGFACSAL
linear_reg() |>
  fit(rank ~ AVGFACSAL, data = scaled_data)

reg <- linear_reg() |>
  fit(rank ~ AVGFACSAL, data = colleges)
reg
...
#ENDOWBEGIN
linear_reg() |>
  fit(log(rank) ~ ENDOWBEGIN, data = scaled_data)

reg <- linear_reg() |>
  fit(log(rank) ~ ENDOWBEGIN, data = colleges)
reg
...
#| label: create_tibble_financials
...
colleges |> 
  ggplot(aes(x = UGDS_HISP, y = rank)) +
  geom_point() +
  labs(
    x = "% of Undergrads Who are Hispanic",
    y = ""
  )
...
colleges |> 
  ...
  geom_point() +
  labs(
    x = "% of Undergrads Who are Asian",
    y = ""
  )

colleges |> 
  ggplot(aes(x = UGDS_AIAN, y = rank)) +
  geom_point() +
  labs(
    x = "% of Undergrads Who are American Indian/Alaskan Native",
    y = ""
  )

colleges |> 
  ggplot(aes(x = UGDS_2MOR, y = rank)) +
  geom_point() +
  labs(
    x = "% of Undergrads Who are 2 or More Races",
    y = ""
  )

colleges |> 
  ggplot(aes(x = UGDS_NRA, y = rank)) +
  geom_point() +
  labs(
    x = "% of Undergrads Who are Non-resident Aliens",
    y = ""
  )

colleges |> 
  ggplot(aes(x = UGDS_UNKN, y = rank)) +
  geom_point() +
  labs(
    x = "% of Undergrads Whose Race is Unknown",
    y = ""
  )
...
scaled_data |> 
  ...
  geom_point() +
  labs(
    x = "Cost of Attendance (academic-year)", 
    y = "",
    #title = "Rank vs Cost of Attendance by Public/Private"
  ) 

scaled_data |> 
  ggplot(aes(x = COSTT4_P, y = rank)) +
  geom_point() +
  labs(
    x = "Cost of Attendance (program-year)", 
    y = ""
  ) 
...
scaled_data |> 
  ...
  geom_point() +
  labs(
    x = "% of First Generation Students",
    y = ""
  ) 
...
data_set <- scaled_data |>
  select(c("college", "rank", "REGION", "CONTROL", "CCBASIC", "ACCREDAGENCY", "ADM_RATE", "UGDS", "UGDS_WHITE", "UGDS_BLACK", "UGDS_HISP", "UGDS_ASIAN", "UGDS_AIAN", "UGDS_NHPI", "UGDS_2MOR", "UGDS_NRA", "UGDS_UNKN", "COSTT4_A", "AVGFACSAL", "PCTPELL", "AGE_ENTRY", "FAMINC", "ENDOWBEGIN", "SAT_AVG", "FEMALE", "MARRIED", "FIRST_GEN"))
...
```{r}
#| label: rank-vs-accredation-agency



colleges_levels |>
  ggplot(aes(y = level, fill = REGION)) +
  geom_bar(position = "fill")

colleges_levels |>
  ggplot(aes(y = level, fill = str_wrap(ACCREDAGENCY, 30))) +
  geom_bar(position = "fill")
```

```{r}
#| label: rank-vs-control



colleges_levels |>
  ggplot(aes(y = level, fill = CONTROL)) +
  geom_bar(position = "fill")
```

```{r}
#| label: rank-vs-CCBASIC


colleges_levels |>
  ggplot(aes(y = level, fill = str_wrap(CCBASIC, 30))) +
  geom_bar(position = "fill")
```
...
```{r}
#| label: adm-rate-vs-rank

colleges |>
  ggplot(aes(x = ADM_RATE, y = rank)) +
  geom_point() +
  geom_smooth(method = "lm") + 
  labs(x = "Admission Rate", y = "Rank", 
       title = "Rank vs Admission Rate") +
  scale_x_continuous(labels = percent)
```

```{r}
#| label: r-squared-adm-rate-vs-rank

adm_rate_rank_fit <- linear_reg() |>
  fit(rank ~ ADM_RATE, data = train)

glance(adm_rate_rank_fit) |>
  select(r.squared) |> knitr::kable()
```
...
colleges |>
  ...
  geom_point() +
  geom_smooth(method = "lm") +
  labs(x = "Undergraduate Enrollment", y = "Rank", 
       title = "Rank vs Undergraduate Enrollment")

```

```{r}
#| label: r-squared-ugds-vs-rank

ugds_rank_fit <- linear_reg() |>
  fit(rank ~ UGDS, data = train)

glance(ugds_rank_fit) |>
  select(r.squared) |> knitr::kable()
...
colleges |>
  ...
  geom_smooth(method = "lm") +
  ylim(0,500) +
  labs(x = "Average Faculty Salary", y = "Rank",
       title = "Rank vs Average Faculty Salary")

```

```{r}
#| label: r-squared-avgfacsal-vs-rank

avgfacsal_rank_fit <- linear_reg() |>
  fit(rank ~ AVGFACSAL, data = train)

glance(avgfacsal_rank_fit) |>
  select(r.squared) |> knitr::kable()
```
...
```{r}
#| label: sat-avg-vs-rank

colleges |>
  ggplot(aes(x = SAT_AVG, y = rank)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(x = "Average SAT Score", y = "Rank", 
       title = "Rank vs Average SAT Score")
...
```{r}
#| label: r-squared-sat-vs-rank

sat_rank_fit <- linear_reg() |>
  fit(rank ~ SAT_AVG, data = train)
