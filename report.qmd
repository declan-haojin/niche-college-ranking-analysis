...
4.  Scale each of the 6 explanatory variables to a normal distribution (with mean at 0 and standard deviation of 1). This step is done because the explanatory variables are on varying scales. By scaling the variables to a normal distribution, it allows us to compare their respective slopes. (THIS IS IN THE DATA PREP ALREADY --- CAN WE DELETE?)
...
As we cannot find an R-squared value or other numerical metric to measure a relationship involving a categorical variable, we decided to do a qualitative variable of our four categorical variables. We created a bar plot for each and filled by rank category (1-100, 101-200, etc.) so we could compare proportions across levels.
...
There does not appear to be a pattern in the accreditation agency graph, which could be because some of the agencies corresponded to very few schools in the top 500. Additionally, as accreditation agency is often based on location, it reflects results similar to the region graph.
...
Qualitatively, it appears that region and Carnegie Classification have the clearest relationship with rank; therefore, we expect `REGION` and `CCBASIC` to show up in the final model.
...
There are three main steps in this approach. First, we need to find out all the highly correlated variable pairs and remove one of them from each pair since we need to rule out the negative impacts of the singularities of variables. Second, we will compute the stepwise regression model using MASS package and mainly stepAIC() functions for model selections based on AIC. For the initial setting of the linear regression model, we will import all the valid variables into the model to predict the rank variable. This will give us a best final model with much fewer variables. Those variables are the most influential factors to the rank of the college.

### Remove variables with high correlations

Our first step in creating a stepwise regression model was to check correlation coefficients between variables so we didn't use variables that were too similar in our model. Then, after removing all columns that had more than 100 NA, we determined which variables had an absolute value of r greater than 0.8. This showed that they were too similar in how they factored into rankings, so we only picked one of them to put in the model.
...
The correlation matrix above provides a visual representation of the correlation between all variable pairs. We want to remove the pairs with a high correlation (absolute value greater than 0.8) which visually are the darker blue squares. However, to use proper statistical analysis, we filtered our correlation matrix to only display entries with values greater or equal to 0.8.
...
### Final Model Results (TODO for Sam)
...
stepwise_model$anova |>
  ggplot(aes(x = stepwise_model$anova$Step, y = stepwise_model$anova$AIC, group = 1, label = stepwise_model$anova$Step)) +
  geom_point() +
...
# aic_value = c(3430.917,3428.988,3427.210,3426.044,3425.339,3423.38,3421.751,3419.900,3419.152,3418.889,3418.618,3418.567)
# variables = c("-FAMINC", "-PCTPELL", "-ENDOWBEGIN", UGDS NHPI UGDS AIAN UGDS NRA UGDSHISP UGDS BLACK UGDS_ WHITE 一 FEMALE + UGDS_ HISP)
```
